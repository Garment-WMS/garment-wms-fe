...
export const productVariantApi = {
  ...
  getOneDisposeHistory: (id: string, queryString: string) =>
    get(`${productVariantPath}/${id}/history/disposed${queryString}`),
};
...
export const getAllProductVariantDisposeFn = async ({
  sorting,
  columnFilters,
  pagination
}: InputType): Promise<ProductVariantResponse> => {
  ...
  // Make the API request
  const res = await privateCall(productVariantApi.getAllDisposeProductVariant(queryString));
  return res.data;
};
...
export const getOneProductDisposeReceiptFn = async (
  id: string,
  { sorting, columnFilters, pagination }: InputType
): Promise<ProductReceiptResponse> => {
  const limit = pagination.pageSize;
  ...
  // Make the API request
  const res = await privateCall(productVariantApi.getOneDisposeReceipt(id, queryString));
  return res.data;
};
...
export const getOneProductHistoryDisposed = async (
  id: string,
  { sorting, columnFilters, pagination }: InputType
): Promise<ProductHistoryResponse> => {
  const limit = pagination.pageSize;
  ...
  // Make the API request
  const res = await privateCall(productVariantApi.getOneDisposeHistory(id, queryString));
  return res.data;
};
