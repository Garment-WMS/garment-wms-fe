...
import { importProductionPlan } from '@/api/services/productionPlan'; // Adjust the path if necessary
...
const errorMessages = {
  ...
  worksheetNotFound: {
    ...
    clientMessage:
      'This is not the production plan worksheet. Please use the correct template and try again.'
  }
};

const UploadExcelProductionPlan: React.FC = () => {
  ...
  const uploadFileToServer = async (file: File) => {
    try {
      const response = await importProductionPlan(file);
      if (response.statusCode !== 200 && response.statusCode !== 201) {
        ...
        if (response?.data?.code) {
          setPlanCode(response?.data?.code);
        }
      }
    } catch (error: any) {
    ...
    <main className="flex flex-col justify-center items-center p-6 bg-white rounded-md shadow-md space-y-6">
      ...
      <div className="text-center">
        <h1 className="font-bold text-xl text-green-600">
          Production Plan: {planCode} uploaded successfully!
        </h1>
        <p className=" text-gray-500 mt-2">
          Your production plan has been uploaded successfully. You can now proceed to view or manage
          it.
        </p>
      </div>
    </main>
      ...
      <DialogContent>
        <DialogTitle>
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-semibold capitalize">Upload Production Plan</h1>
            <DialogClose />
          </div>
        </DialogTitle>
...
export default UploadExcelProductionPlan;
