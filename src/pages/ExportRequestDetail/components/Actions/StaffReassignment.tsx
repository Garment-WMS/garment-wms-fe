...
import { reassignStaffFn } from '@/api/purchase-staff/exportRequestApi';
import { useParams } from 'react-router-dom';
import { reAssignStaffForExportRequest } from '@/api/services/exportRequestApi';
...
export default function ReassingStaffPopup({
  ...
  return (
    <Dialog
      ...
      <DialogContent className="sm:max-w-[825px] h-[90%]">
        {!isLoading ? (
          <ScrollArea>
            <DialogHeader>
            ...
            <Card className="rounded p-2  w-[40%]">
              <div className="flex items-center">
                <Avatar className="mr-4">
                  <AvatarImage src={exportRequest?.warehouseStaff.account.avatarUrl} />
                  <AvatarFallback>
                    {exportRequest?.warehouseStaff.account.firstName.slice(0, 1) +
                      exportRequest?.warehouseStaff.account.lastName.slice(0, 1)}
                  </AvatarFallback>
                </Avatar>
              ...
              <div className={staff ? 'line-through' : ''}>
                {new Date(exportRequest?.expectedStartedAt)
                ...
                {' - '}
                {new Date(exportRequest?.materialExportReceipt?.expectedFinishedAt)
                  .toLocaleString('en-GB', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit',
